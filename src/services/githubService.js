...
async function ensureFeatureBranch({ owner, repo, defaultBranch, featureBranch, userToken }) {
    // Prefer an explicit user token when provided, otherwise fall back
    // to the active user token managed by setActiveToken().
    const tokenToUse = userToken || getActiveToken();
    const client = getOctokit(tokenToUse);
...
async function createPullRequestForWorkflow({ repoName, filePath, content, language, issueKey, deployTarget, defaultBranch, repoConfig, ticketData, aiAnalysis }) {
    try {
        ...
        // Use the active OAuth token for all branch/PR operations.
        // This is set in server.js when the user logs in via GitHub.
        const userToken = getActiveToken();
        ...
        // 3. Ensure Branch Exists (with user token)
        await ensureFeatureBranch({ owner, repo, defaultBranch, featureBranch, userToken });
...
// --- Org-level PR discovery & GitHub Issue key mapping ---
...
async function checkAppInstallation(token) {
    ...
    try {
        ...
        // Default to the public Sentinel app slug, overrideable via env
        const appSlug = process.env.GITHUB_APP_SLUG || 'sentinel-devops-automation-agent';
